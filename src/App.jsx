...
import {
  ...
  Palette, Sofa, Lightbulb, Layers, Frame, Wind, RotateCcw
} from "lucide-react";
...
RECOMMENDATIONS: non-structural ONLY (no wall/window/door/ceiling/floor-plan changes). Categories: color, furniture, lighting, texture, styling, mood. Each rec links to the principle it fixes. Tag impact: "High" | "Medium" | "Nice-to-Have". 8-12 recs total across categories. CRITICAL: each rec "text" MUST be 200 characters or fewer — one tight, specific sentence, no filler.

NANO BANANA PRO PROMPT: ONE single flowing English paragraph (NO labels, NO line breaks) implementing the High + Medium recs. CRITICAL: it MUST be 500 characters or fewer. Pack in the key edits (color, lighting, furniture, styling), then end with a short keep/constraint clause like "keep the original room structure, camera angle and proportions; no structural changes, no text." Always in English. Count characters and stay under 500.
...
Respond with ONLY a valid JSON object (no markdown, no backticks, no preamble) in EXACTLY this shape:
{
 ...
 "overall":0,
 "tier":"Professional",
 "summary":"verdict naming strongest + weakest point",
 "recommendations":{"color":[{"text":"","impact":"High"}],"furniture":[],"lighting":[],"texture":[],"styling":[],"mood":[]},
 "prompt":"single flowing English paragraph, max 500 chars"
}
LENGTH LIMITS (strict): principle "note" ≤ 120 chars; elegance "note" ≤ 200 chars; each rec "text" ≤ 200 chars; "summary" ≤ 220 chars; "prompt" ≤ 500 chars. Be clear and specific but cut all filler. The "tier" value MUST be one of: Masterclass, Professional, Competent, Developing, Needs Rework (English label).`;
}
  ...
  async function analyze(b64, mediaType, useLang) {
    ...
    const attempt = async () => {
      ...
      try {
        ...
        if (data.stop_reason === "max_tokens") {
          throw new Error("The analysis was cut off (response too long).");
        }
    ...
    try {
      ...
      try {
        ...
        // One automatic retry — the model occasionally returns malformed/truncated JSON.
        console.warn("First attempt failed, retrying:", firstErr);
        parsed = await attempt();
      }
      setResult(sanitizeResult(parsed));
    } catch (e) {
            ...
            <div className="izj-card" style={{ padding: 22, marginTop: 30, border: "1px solid rgba(201,168,106,0.32)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 14, flexWrap: "wrap" }}>
                <div>
                  ...
                  <p style={{ margin: "6px 0 0", color: "#a89b81", fontSize: 13 }}>
                    {t.promptSub}
                    <span style={{ color: "#8a7d64", marginInlineStart: 8 }} dir="ltr">
...
function sanitizeResult(r) {
  const out = { ...r };
  if (Array.isArray(out.core)) out.core = out.core.map((x) => ({ ...x, note: clamp(x.note, 120) }));
  if (Array.isArray(out.elements)) out.elements = out.elements.map((x) => ({ ...x, note: clamp(x.note, 120) }));
  if (Array.isArray(out.elegance)) out.elegance = out.elegance.map((x) => ({ ...x, note: clamp(x.note, 200) }));
  out.summary = clamp(out.summary, 220);
  out.prompt = clamp(out.prompt, 500);
  if (out.recommendations && typeof out.recommendations === "object") {
    const recs = {};
    for (const [k, arr] of Object.entries(out.recommendations)) {
      recs[k] = Array.isArray(arr) ? arr.map((rec) => ({ ...rec, text: clamp(rec.text, 200) })) : arr;
    }
